...
class PipelineUI(QtWidgets.QMainWindow):
    ...
    def searchInJsonList(self):
        #self.analysis = JsonFiles()
        #self.pipe = PipelineUI()
        self.readFile = self.read.readInJson()
    ...
    def searchAndDropWidget(self):
        ...
        self.searchLineEdit.textChanged.connect(self.searchNDropInteract)
    ...
    def searchNDropInteract(self):
        ...
        self.model.index(self.path)
        '''


        # Set the model, the root index and the drop possibility.
        self.searchTreeView.setModel(self.modelCustom)
        ...
        if(self.text == ''):
            pass
            self.modelCustom.get_contents(listContents = self.read.readInJson())
            #self.searchTreeView.expandAll()
            #print("base tree structure")
        else:
            #print("expanded")
            self.modelCustom.get_contents(listContents = self.matchingList)
            self.searchTreeView.expandAll()

        self.searchTreeView.clicked.connect(self.test)
        

    def test(self, signal):
        filePath = self.modelCustom.filePath(signal)
        print(filePath)
