...
class MayaExports():
    ...
    def exportMA(self, checkBox, tOrP, aOrS, endPath, seq, shot):
        ...
        if(checkBox.isChecked() == True):                                               # If one checkbox is checked.
            ...
            for each in self.mySel:                                                     # Loop over the selection.
                ...
                files = [f for f in os.listdir(self.pathExport) if any(f.endswith(ext) for ext in includeExt) and any(f.startswith(s) for s in sel)] # Comprehensive list to filter files in the dir pathExport by extension and outliner names.
    ...
    def exportOBJ(self, checkBox, tOrP, aOrS, endPath, seq, shot):
    ...
    def exportABC(self, checkBox, tOrP, aOrS, startFrame, endFrame, subA, subB, attrExport, endPath, seq, shot):
